...
:description: Table reference for metabib.title_field_entry
...
[WARNING]
====
*Deferrable Constraints:* The following FK constraints are deferrable — they are checked at transaction end, not statement end: `metabib_title_field_entry_field_pkey`, `metabib_title_field_entry_source_pkey`.
====
...
| `bigint`
| No
| nextval('metabib.title_field_entry_id_seq'::regclass)
| 
...
| `title_field_entry_pkey` [.label]#PK#
| btree
| `CREATE UNIQUE INDEX title_field_entry_pkey ON metabib.title_field_entry USING btree (id)`
