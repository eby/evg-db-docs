= container.biblio_record_entry_bucket_note
:description: Table reference for container.biblio_record_entry_bucket_note

xref:schemas/container.adoc[↑ container schema]

[WARNING]
====
*Cascading Deletes:* Deleting rows from this table will cascade to: xref:tables/container/container_biblio_record_entry_bucket.adoc[container.biblio_record_entry_bucket].
====

[WARNING]
====
*Deferrable Constraints:* The following FK constraints are deferrable — they are checked at transaction end, not statement end: `biblio_record_entry_bucket_note_bucket_fkey`.
====

== Columns

[cols="2,2,1,3,4"]
|===
|Column |Type |Nullable |Default |Notes

| *id* [.label]#PK#
| `integer`
| No
| nextval('container.biblio_record_entry_bucket_note_id_seq'::regclass)
| 

| bucket [.label]#FK#
| `integer`
| No
| 
| → xref:tables/container/container_biblio_record_entry_bucket.adoc[container.biblio_record_entry_bucket(id)]

| note
| `text`
| No
| 
| 

|===

== Primary Key

`(id)`

== Foreign Keys

[cols="2,3,1,1,1,2"]
|===
|Column(s) |References |On Delete |On Update |Deferrable |Constraint

| `bucket`
| xref:tables/container/container_biblio_record_entry_bucket.adoc[container.biblio_record_entry_bucket(id)]
| [.label]#CASCADE#
| CASCADE
| DEFERRED
| `biblio_record_entry_bucket_note_bucket_fkey`

|===

== Indexes

[cols="3,1,5"]
|===
|Index |Method |Definition

| `biblio_record_entry_bucket_note_pkey` [.label]#PK#
| btree
| `CREATE UNIQUE INDEX biblio_record_entry_bucket_note_pkey ON container.biblio_record_entry_bucket_note USING btree (id)`

|===

