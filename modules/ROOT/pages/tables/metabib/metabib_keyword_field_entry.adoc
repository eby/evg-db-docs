...
:description: Table reference for metabib.keyword_field_entry
...
[WARNING]
====
*Deferrable Constraints:* The following FK constraints are deferrable — they are checked at transaction end, not statement end: `metabib_keyword_field_entry_field_pkey`, `metabib_keyword_field_entry_source_pkey`.
====
...
| `bigint`
| No
| nextval('metabib.keyword_field_entry_id_seq'::regclass)
| 
...
| `keyword_field_entry_pkey` [.label]#PK#
| btree
| `CREATE UNIQUE INDEX keyword_field_entry_pkey ON metabib.keyword_field_entry USING btree (id)`
