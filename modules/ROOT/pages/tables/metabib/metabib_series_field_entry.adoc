...
:description: Table reference for metabib.series_field_entry
...
[WARNING]
====
*Deferrable Constraints:* The following FK constraints are deferrable — they are checked at transaction end, not statement end: `metabib_series_field_entry_field_pkey`, `metabib_series_field_entry_source_pkey`.
====
...
| `bigint`
| No
| nextval('metabib.series_field_entry_id_seq'::regclass)
| 
...
| `series_field_entry_pkey` [.label]#PK#
| btree
| `CREATE UNIQUE INDEX series_field_entry_pkey ON metabib.series_field_entry USING btree (id)`
