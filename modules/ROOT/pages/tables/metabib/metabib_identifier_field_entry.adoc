...
:description: Table reference for metabib.identifier_field_entry
...
[WARNING]
====
*Deferrable Constraints:* The following FK constraints are deferrable — they are checked at transaction end, not statement end: `metabib_identifier_field_entry_field_pkey`, `metabib_identifier_field_entry_source_pkey`.
====
...
| `bigint`
| No
| nextval('metabib.identifier_field_entry_id_seq'::regclass)
| 
...
| `identifier_field_entry_pkey` [.label]#PK#
| btree
| `CREATE UNIQUE INDEX identifier_field_entry_pkey ON metabib.identifier_field_entry USING btree (id)`
